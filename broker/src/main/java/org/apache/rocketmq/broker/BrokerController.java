...
public class BrokerController {
    ...
    public BrokerController(
        ...
        //下面是broker的一些功能性组件
        //StatsManager一看就是metric统计组件
        //brokerfastFailure用于处理broker故障的组件
    ...
    public boolean initialize() throws CloneNotSupportedException {
        //加载一些磁盘上的数据到内存
        //比如加载topic的配置、consumer的消费offset
        //consumer订阅组、过滤器
        ...
        if (result) {
            try {
                ...
                //若启用dleger技术进行主从同步以及管理commitlog
                //初始化一堆dleger相关的组件
        ...
        if (result) {
            ...
            //设置nameserver地址列表，只不过支持不通过配置的方式来写入nameserver地址，还可以让他发送请求去加载
    ...
    public synchronized void registerBrokerAll(final boolean checkOrderConfig, boolean oneway, boolean forceRegister) {
        ...
        //判断了一下，是否要进行注册
        //如果要进行注册的话，就调用了doRegisterBrokerAll()方法去真正的注册
    ...
    private void doRegisterBrokerAll(boolean checkOrderConfig, boolean oneway,
        ...
        //为什么注册结果是list？因为broker会把自己注册到所有的nameserver
        ...
        //如果注册结果数量大于0，下面对注册结果进行处理
        //处理逻辑设计到MasterHAServer之类的东西，先不用深究
